...
def enrich_history_with_factors(history: pd.DataFrame, factor_names: list[str]) -> pd.DataFrame:
    """
    在 OHLCV history 上计算所需因子列。
...
def eval_factor_rules(data: dict[str, Any], ctx: Any) -> list[Signal]:
    ...
    enriched = enrich_history_with_factors(history, names)
