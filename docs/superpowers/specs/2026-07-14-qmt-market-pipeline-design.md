...
> 状态：已确认（brainstorming §1–§3）；**2026-07-14 增补**：日线起始日、退市过滤、前复权默认列 + 后复权 `_hfq`（待用户再审）  
...
| 日线表仅一套 OHLCV | 现有 `BarDaily`：`open/high/low/close/volume/amount` + `adj_factor`；本迭代需扩展为**默认列=前复权，另存后复权 `_hfq` 列**（见 §4.3） |
...
### 4.3 前复权（默认列）+ 后复权（`_hfq`）同键落库

**现状**：`BarDaily` 仅一套 `open/high/low/close/volume/amount` + `adj_factor`。

**命名约定（宽表同键，避免 `(symbol, ts, adj_type)` 复杂化唯一键）**：

| 字段组 | 含义 |
|--------|------|
| `open`, `high`, `low`, `close`, `volume`, `amount` | **前复权**（无后缀；策略/回测/API 默认读这些列） |
| `open_hfq`, `high_hfq`, `low_hfq`, `close_hfq`, `volume_hfq` | **后复权**（仅 `_hfq` 后缀；`amount` 是否另存 `amount_hfq` 以实现时与源对齐为准，默认可与默认 `amount` 共用） |
| （可选保留）`adj_factor` | 若仍有单因子用途可保留；**不以**「一套价 × 因子」代替后复权列落库 |

**重要**：前复权**不**使用 `_qfq` 后缀；列名本身无 `_qfq`。

迁移要点：

- 唯一键仍为 `(symbol, ts)`；upsert 同键覆盖默认前复权列与 `_hfq` 列。
- 既有 `open/high/low/close/volume/amount`：**语义定为前复权**；新增 `_hfq` 列（Alembic/SQL 步骤在实现计划写清）。无需把默认列改名为 `*_qfq`。
- **未复权价**：本期**可以不存**（除非实现时发现与前/后复权同一次拉取几乎零成本，则可选落库；默认范围不含未复权）。

**消费约定**：

| 场景 | 默认 | 可切换 |
|------|------|--------|
| 策略选股 / 回测 | **前复权默认列** `open/high/low/close/volume` | 配置或参数切 **后复权 `_hfq` 列** |
| DataFeed → backtrader `PandasData` | 直接映射默认列 → open/high/low/close | 可配置改为映射 `*_hfq` |
| `GET /bars/daily` | 不传或 `adj` 默认 = 前复权（默认列）；`adj=hfq` → 读 `_hfq` 列；可选保留 `adj=qfq` 作语义别名（仍映射默认列，**非**列后缀） | 查询参数或响应字段约定在实现时固化 |

拉取侧：QMT/`xtdata` 日线请求需显式取前复权与后复权（或等价两次/带复权参数一次解析）；写入时前复权进默认列、后复权进 `_hfq`；AkShare 补洞路径对齐同一字段语义后再 upsert。
...
- **复权**：分钟本迭代**不要求**前/后复权双列落库（体量与用途以盘中监控为主）；若展示涨跌依赖日线，仍读 `bars_daily` 默认前复权列。

## 6. 组件与职责

| 组件 | 职责 |
|------|------|
| `qmt_md`（建议路径 `packages/market/.../qmt_md`） | 封装 `xtdata`：股票列表（含退市/InstrumentStatus）、日线批量（前复权→默认列 + 后复权→`_hfq`）、分钟、快照只读。**与 `QmtBroker` 交易通道严格分离** |
| 证券列表同步 | 过滤退市；产出「可交易/在市」全 A 宇宙供日终与回填；可选标记库内退市标的 |
| `DailyBarIngestor` | 全 A（在市）日终增量 → `upsert` `bars_daily`（默认列 + `_hfq`） |
...
| DataFeed（回测边界） | 默认列（前复权）→ `PandasData`；可配置改读 `*_hfq` |
| APScheduler | 挂在 FastAPI `lifespan`：启动注册、关闭 shutdown |

既有 `MarketService.upsert_daily_bars` 需扩展为同键写入默认前复权列与 `_hfq` 列；分钟侧对称增加 upsert 与 purge。
...
证券列表(xtdata) ──过滤退市──► 全 A 在市宇宙
                             │
QMT xtdata ──日线(前复权+hfq)──► DailyBarIngestor ──upsert──► bars_daily（全 A 在市，长期；≥ daily_start_date）
       │                    ▲
...
消费：选股/回测/DataFeed 默认读无后缀前复权列；可配置读 `*_hfq`
...
| `ingest_daily_incremental` | 约 15:30（交易日） | QMT 全 A **在市** 当日/近 N 日日线（前复权→默认列 + `_hfq`）upsert；**不**为起始日回补历史 |
...
- 日线 / 分钟唯一键均为 `(symbol, ts)`；写入一律 **upsert**（同键覆盖默认前复权 OHLCV 与 `_hfq` 列 / 分钟 OHLCV，不堆重复行；**不**采用 `adj_type` 分行）。
- 任务可重入：重复执行日终增量不增加行数。
...
| 复权某一侧缺失 | 记错误列表；实现策略二选一并在代码注记：整行跳过 **或** 允许默认列/`_hfq` 一侧先写、待补（推荐：同行默认列与 `_hfq` 齐备再 upsert，避免半截复权行） |
...
- seed 演示数据应写入**默认前复权列与 `_hfq` 列**（可用相同数字填两侧）以免读路径报缺列。
...
| `GET` | `/bars/daily?symbol=&from=&to=&adj=` | 读 `bars_daily`；`adj` 省略或默认 = 前复权（默认列）；`adj=hfq` → `_hfq` 列；可选 `adj=qfq` 作语义别名仍指向默认列（列名无 `_qfq`） |
...
1. 交易日日终（或手动 `daily-sync`）后，**在市**任务宇宙内标的的 **当日日线**可在 `bars_daily` 查到，且同行含 **前复权默认列与后复权 `_hfq` 列**；重复执行 **不增重复行**。
...
5. 监控/API：日线读库（默认前复权列）；分钟读库；最新价可选 QMT 补洞。
6. DataFeed/回测默认映射无后缀前复权列；配置可切 `*_hfq`。
7. QMT 断开时任务失败可观测（status/日志）；已有数据仍可展示。
8. pytest：upsert 幂等（默认列 + `_hfq`）、3 交易日 purge、缺口回填下界、退市过滤（Mock InstrumentStatus）、QMT→AkShare 冒烟通过。
...
- 以 `adj_type` 分行存复权（已否决；宽表：默认列=前复权 + `_hfq`=后复权）
...
- Mock `qmt_md`：日线返回固定前复权与 hfq 序列 → 双次 sync 行数不变，默认列与 `_hfq` 被覆盖更新。
...
- DataFeed：默认读无后缀列；切 `hfq` 后映射 `*_hfq`。
- 日历缺表时门闸日志含「日历未同步」；有真实日历后节假日不触发日终增量。

---

**下一步**：请用户再审阅本节增补（起始日、退市、默认列=前复权 / `_hfq`=后复权）通过后，再编写实施计划（writing-plans），然后开写代码。
