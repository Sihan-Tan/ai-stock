...
describe("groupFactorCatalog", () => {
  it("lists selected and puts all factors under categories", () => {
    ...
    expect(g.collapsedCategories.find((c) => c.category === "trend")!.items.map((i) => i.name)).toEqual([
      "SMA_20",
      "SMA_10",
    ]);
    expect(g.collapsedCategories.find((c) => c.category === "momentum")!.items.map((i) => i.name)).toEqual([
      "RSI_14",
      "WILLR_14",
    ]);
  });
